...
func TestAudioHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T, st *services.LocalDiskStorage) {
				mustStage(t, st, validID, "shifted/-2/audio"+services.AudioExt, []byte("fake mp3 bytes"))
			},
		...
		},
		{
			...
			setup: func(t *testing.T, st *services.LocalDiskStorage) {
				mustStage(t, st, validID, "shifted/0/audio"+services.AudioExt, []byte("zero semitones"))
			},
			wantStatus: http.StatusOK,
		...
		},
		{
			...
			setup: func(t *testing.T, st *services.LocalDiskStorage) {
				mustStage(t, st, validID, "shifted/5/audio"+services.AudioExt, []byte("plus five"))
			},
			wantStatus: http.StatusOK,
			wantBody:   "plus five",
		},
...
func TestAudioHandler_RangeRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mustStage(t, st, validID, "shifted/0/audio"+services.AudioExt, content)

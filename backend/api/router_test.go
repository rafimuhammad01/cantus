...
func TestRouter_Health(t *testing.T) {
	...
	router := api.NewRouter([]string{"http://localhost:5173"}, log, &fakeYouTubeService{}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
...
func TestRouter_blobRoute_mountedOnlyWhenTokenerSet(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			mux := api.NewRouter([]string{"http://localhost:5173"}, zerolog.Nop(), nil, signer, storage, nil, nil, nil, store, tc.tokener, nil, nil, nil)

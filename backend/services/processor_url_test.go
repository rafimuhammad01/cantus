...
func TestPythonCPUProcessorClient_Shift(t *testing.T) {
	tests := []struct {
		name        string
		inputURL    string
		outputURL   string
		...
		{
			name:      "happy path sends correct body and path",
			inputURL:  "https://r2/in.wav",
			outputURL: "https://r2/out.mp3",
			semitones: -3,
			handler: func(w http.ResponseWriter, r *http.Request) {
				...
				if body.InputURL != "https://r2/in.wav" {
					t.Errorf("input_url: got %q, want https://r2/in.wav", body.InputURL)
				}
				if body.OutputURL != "https://r2/out.mp3" {
					t.Errorf("output_url: got %q, want https://r2/out.mp3", body.OutputURL)
				}
				if body.Semitones != -3 {
					t.Errorf("semitones: got %v, want -3", body.Semitones)
				}
		...
		{
			name:      "upstream 502 returns error with status code",
			inputURL:  "u",
			outputURL: "v",
			semitones: 0,
			handler: func(w http.ResponseWriter, r *http.Request) {
		...
		{
			name:      "context canceled returns context.Canceled",
			inputURL:  "u",
			outputURL: "v",
			semitones: 0,
			// handler receives a pre-canceled ctx, so request never reaches server
			handler: func(w http.ResponseWriter, r *http.Request) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.errIs == context.Canceled {
				var cancel context.CancelFunc
				ctx, cancel = context.WithCancel(ctx)
				cancel()
			}

			err := c.Shift(ctx, tt.inputURL, tt.outputURL, tt.semitones)
			...
			if tt.wantErr || tt.errIs != nil {
				...
				if tt.errIs != nil && !errors.Is(err, tt.errIs) {
					t.Errorf("error %v: expected errors.Is(err, %v) to be true", err, tt.errIs)
				}
				return
			}
...
func TestPythonCPUProcessorClient_PreviewKey(t *testing.T) {
	tests := []struct {
		name        string
		inputURL    string
		handler     http.HandlerFunc
	...
	}{
		{
			...
			handler: func(w http.ResponseWriter, r *http.Request) {
				...
				if body.InputURL != "https://r2/in.mp3" {
					t.Errorf("input_url: got %q, want https://r2/in.mp3", body.InputURL)
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			key, err := c.PreviewKey(context.Background(), tt.inputURL)
...
func TestPythonGPUProcessorClient_Separate(t *testing.T) {
	tests := []struct {
		name              string
		inputURL          string
...
func TestPythonGPUProcessorClient_Melody(t *testing.T) {
	tests := []struct {
		name           string

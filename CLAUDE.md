...
- **ProcessorClient is split** into `CPUProcessorClient` (Shift, PreviewKey) and `GPUProcessorClient` (Separate, Melody) in `backend/services/processor_url.go`. Both are URL-based — Go mints presigned GET/PUT URLs, Python downloads → processes → uploads. Both currently point at the same Python service via `CPU_PROCESSOR_URL` / `GPU_PROCESSOR_URL` (each defaulting to `PYTHON_PROCESSOR_URL`); the Python service split plan will route them to separate hosts.
